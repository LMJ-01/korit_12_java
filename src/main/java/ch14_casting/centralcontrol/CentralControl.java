...
public class CentralControl {
    ...
    /*
        ...
        우리는 추가하기 전에 일단 배열을 비어있는 곳이 있는지 체크하도록 할 예정입니다.
        그러면 추가하는 메서드를 정의할 떄의 로직은
...
//        }
    ...
    /*
        Java의 index 넘버에는 음수값이 없기 떄문에(python에는 있지만), 실패를 나타낼 때 -1을
        쓰는 경우가 많습니다. 하지만 0과 너무 가까운 수이다보니까 조직에 따라서 -100이라든지
        아니면 return으로 나올 수 없을 만한 음수값을 저장하는 경우도 있는데,
    ...
    public void powerOn() {
        /*
            ...
            반복문을 돌면서 deviceArray[i].on(); 이거 실행시켜주면 전부 다 켜지겠네요.
         */
        for (int i = 0; i < deviceArray.length; i++) {
            if (deviceArray[i] == null) {
                ...
                /*
                    ...
                    continue;는 현재 반복'만' 종료하고 다음 반복문으로 넘어감.
                    즉, deviceArray[6] 이 null 이라면 다음 반복으로 i++시켜서
                    deviceArray[7]을 검사하고 deviceArray.lenght 미만까지
        ...
        /*
            ...
            단 배열 내부를 탐색할 떄 향상된 for문으로 작성하시오.
            장치가 없어 전원을 끌 수 없습니다.
         */
    }
    ...
    */
    ...
    public void deleteDevice(int Num) {
        ...
        if (deviceArray[index] == null) {
            System.out.println("해당 슬롯에는 삭제할 장치가 없습니다.");
            return;
        }

        System.out.println(deviceArray[index].getClass().getSimpleName()
                + " 장치가 삭제되었습니다.");
