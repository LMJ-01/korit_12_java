...
public class CentralControlMain {
    public static void main(String[] args) {
        ...
        // 이하의 코드는 powers 배열을 만든 다음에 내부에 Power의 하위 클래스의 객체를 집어넣었습니다.
        Power[] powers = {new AirConditioner(), new Computer(), new Mouse(), new LED(), new Printer(), new Speaker()};    // 여기서 업캐스팅이 이루어졌습니다.

        /*
            근데 저희는 여기서 각 전자기기들의 객체를 생성하고 여기서 집어넣는 것까지 전체 다 실험할 예정이기 때문에
            powers를 직접 쓰지는 않을거고, Power가 interfacze이기 때문에 객체 생성이 안되는데 배열을 만들 수 있는가를
            검증하기 위해 코드를 남겨두겠습니다.
         */
//        centralControl1.deviceArray 에 접근하는 것이 불가능합니다. 접근 제어가가 private이기 때문에
        // 그렇다면 centralControl1.deviceArray에 Power의 하위 클래스인 AirConditioner 등의 객체를 집어넣기 위해서는
        // public method를 경유해야할 것 같습니다. -> ContralControl 클래스에 public method를 정의해야만 하겠다.
