...
public class Array03Method {
    ...
    public void writeNameAndScore() {
        String[] names = inputNames();          // inputNames() 메서드의 retrun형이 String 배열이니까 배열 선언해서 초기화
        double[] scores = inputScores(names);   // inputScores() 메서드는 String 배열 argument로 요구하니까 포함하고,
                                                // return이 double 배열이니까 writeNameAndScore() 메서드 내에서
                                                // 실수 배열 선언 후 초기화
        System.out.println("학생들의 점수 총합은 " + Arrays.stream(scores).sum() + "입니다.");
        System.out.println("학생들의 점수 평균은 " + Arrays.stream(scores).average().getAsDouble() + "입니다.");
    }

    // 금일 자습 시간 혹은 면담 시간 중에 여러분들이 할 일
    // inputNum() 메서드를 정의해서 배열 의한계값들을 입력 받는 메서드를 구현하고
    ...
    // n 번째 학생의 점수를 입력 하세요 >>> 가 나올 수 있도록 하시오.
